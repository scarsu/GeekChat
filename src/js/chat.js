$(function(){
	function Chat(cfg){
		this.socket = null;//socket client对象
		this.cfg = cfg;//通过参数cfg对象传入页面相关的dom元素
        //缓存昵称
		this.arrName = '';
		//随机头像数组
		this.avatorArr = ['1.jpg','2.jpg','3.jpg','4.jpg','5.jpg','6.jpg','7.jpg','8.jpg','9.jpg','10.jpg','11.jpg','12.jpg','13.jpg'];
		//缓存头像
		this.avatar = '';
		this.level = ['潜水', '冒泡', '吐槽', '活跃', '话痨', '传说', '砖家'];
		this.levelName = '';
		this.init();
	}
	Chat.prototype = {
		init : function(){
			...
			//初始化昵称输入框(登录框)
			if( $.cookie('chatName') == null ){//若cookie昵称不存在，显示输入昵称框
				me.cfg.setName.removeClass('hide');
			}else{//若cookie昵称存在，隐藏输入昵称框，从cookie中取值
				me.setName();
			}
			//初始化头像
			if( $.cookie('avatar') == null ){
				var randAvatar = me.avatorArr[ Math.floor( Math.random()*me.avatorArr.length ) ];
				$.cookie('avatar', randAvatar, { expires : 1 });
			}
			me.avatar = $.cookie('avatar');	
			me.cfg.avatar.attr('src', "./styles/images/" + me.avatar);
			/*me是Chat()类实例化后的chat对象
			* me.cfg*/
		...
		setName : function(){//从cookie获取用户名
			var me = this;
			me.cfg.setName.addClass('hide');//隐藏用户名输入框
			me.arrName = $.cookie('chatName');
			var area = remote_ip_info.province + "·" + remote_ip_info.city;//新浪api获取所在地址
